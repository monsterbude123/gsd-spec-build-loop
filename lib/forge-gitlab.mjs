...
export function createGitLabForge({ cwd, repo, run, env = process.env, host, token }) {
  ...
  // gh 语义的 @me 在 GitLab 侧必须解析为当前认证用户(GET /user);
  // 项目成员搜索不认识 "me",按用户名搜索会静默漏配指派。
  function resolveUser(name) {
    const bare = name.replace(/^@/, "");
    if (bare === "me") {
      return request("GET", "/user");
    }
    const users = request("GET", `/projects/${project}/users?search=${encodeURIComponent(bare)}`);
    return users.find((user) => user.username === bare);
  }
  ...
  return {
    ...
    issueEdit(number, { addAssignee, removeAssignee, addLabels, removeLabels } = {}) {
      ...
      if (addAssignee || removeAssignee) {
        // assignee_ids 是全量替换语义:先读现有指派,再按增删合并。
        const current = request("GET", `/projects/${project}/issues/${number}`);
        let ids = (current.assignees ?? []).map((user) => user.id);
        const removeUser = removeAssignee ? resolveUser(removeAssignee) : undefined;
        const addUser = addAssignee ? resolveUser(addAssignee) : undefined;
        if (addAssignee && !addUser) {
          throw new BlockedError(`GitLab forge could not resolve assignee ${addAssignee}`);
        }
        if (removeUser) {
          ids = ids.filter((id) => id !== removeUser.id);
        }
        if (addUser && !ids.includes(addUser.id)) {
          ids = [...ids, addUser.id];
        }
    ...
    prCreate({ title, body, head, base }) {
      if (!head) {
        throw new BlockedError("GitLab pr-create requires --head BRANCH");
      }
      // gh 语义:base 缺省 = 仓库默认分支
      const targetBranch = base ?? request("GET", `/projects/${project}`).default_branch;
    ...
    prEdit(number, { removeLabels, body, addLabels } = {}) {
      const payload = {};
      if (body !== undefined) payload.description = body;
      if (addLabels?.length || removeLabels?.length) {
        // MR 更新 API 没有 issue 的 add_labels/remove_labels 参数,只认全量 labels;
        // 先读现值再合并/移除,保持与 gh --add-label/--remove-label 相同的增删语义。
        const current = request("GET", `/projects/${project}/merge_requests/${number}`);
        const names = new Set(current.labels ?? []);
        for (const label of removeLabels ?? []) names.delete(label);
        for (const label of addLabels ?? []) names.add(label);
        payload.labels = [...names].join(",");
      }
