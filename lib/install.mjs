  ...
  try {
    ...
    if (["discover", "spec", "build", "review"].includes(lane)) {
      ...
      if (["discover", "spec"].includes(lane)) {
        ...
        const runtime = join(stage, "scripts", "runtime");
        mkdirSync(runtime, { recursive: true });
        for (const module of ["errors.mjs", "forge.mjs", "forge-github.mjs", "forge-gitlab.mjs", "linkage.mjs", "process.mjs"]) {
          cpSync(join(sourceRoot, "lib", module), join(runtime, module));
        }
      } else if (lane === "review") {
        const runtime = join(stage, "scripts", "runtime");
        mkdirSync(runtime, { recursive: true });
        for (const module of ["audit-evidence.mjs", "errors.mjs", "forge.mjs", "forge-github.mjs", "forge-gitlab.mjs", "outcomes.mjs", "process.mjs"]) {
          cpSync(join(sourceRoot, "lib", module), join(runtime, module));
        }
      }
    }
