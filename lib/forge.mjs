...
export function createForge({
  ...
  if (!resolvedForge) {
    throw new UsageError(
      "could not detect forge: git remote get-url origin failed; set it explicitly, e.g. GSD_LOOP_FORGE=gitlab or GSD_LOOP_FORGE=github",
    );
  }
...
function readBodySource({ body, bodyFile, "body-file": bodyFileDashed } = {}) {
  const file = bodyFile ?? bodyFileDashed;
  if (file) {
