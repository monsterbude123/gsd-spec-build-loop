...
export function parseOutcomeArguments(values) {
  ...
  for (let index = 0; index < options.length; index += 1) {
    ...
    if (!["--repo", "--pr", "--head", "--contract"].includes(option) || !value || value.startsWith("--")) {
    ...
    if (option === "--contract") {
      if (!/^[0-9a-f]{64}$/.test(value)) throw new UsageError("--contract requires SHA256");
      parsed.expectedContract = value;
    }
    index += 1;
  }
...
export function fingerprintContract(body) {
  return createHash("sha256").update(transformOutcomeChecklist(body, "pending")).digest("hex");
}

function pullRequestEvidence({ cwd, repo, pullRequest, run }) {
  // GitHub backend 默认字段与历史实现一致:headRefOid,body,closingIssuesReferences。
  return createForge({ cwd, repo, run, fallbackForge: "github", detect: false }).prView(pullRequest);
}

function issueBody({ cwd, repo, issue, run }) {
  return createForge({ cwd, repo, run, fallbackForge: "github", detect: false }).issueBody(issue);
}
...
export function syncIssueOutcomes({
  ...
  if (expectedContract !== undefined && fingerprintContract(original) !== expectedContract) {
    throw new BlockedError(`issue #${issue} contract changed; verdict evidence is stale`);
  }
  ...
  createForge({ cwd, repo, run, fallbackForge: "github", detect: false }).issueBodyEdit(issue, updated);
