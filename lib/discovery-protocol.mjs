...
// The repository issues list endpoint is eventually consistent: an issue this
// helper just created is reliably absent from it for a few seconds even though
// the create returned successfully and a direct read of that issue succeeds.
// Re-read the snapshot with bounded backoff whenever the caller can state what
// the write it just made must make visible.
const ISSUE_SNAPSHOT_BACKOFF_MS = [1000, 2000, 4000, 8000, 8000, 8000];
...
function readRepositoryIssues(options) {
...
function repositoryIssues(options, { refresh = false, until = null } = {}) {
  if (!refresh && options.issueSnapshot) return options.issueSnapshot;
  options.issueSnapshot = readRepositoryIssues(options);
  if (!until) return options.issueSnapshot;
  const wait = options.sleep ?? sleepSync;
  for (const backoff of ISSUE_SNAPSHOT_BACKOFF_MS) {
    if (until(options.issueSnapshot)) break;
    wait(backoff);
    options.issueSnapshot = readRepositoryIssues(options);
  }
...
export function fileDiscoverySlice({
  cwd,
  repo,
  map,
  slice,
  ...
  const options = { cwd, repo, map, run, sleep };
  ...
  if (!matches.length) {
    ...
    matches = matchingSliceIssues(
      repositoryIssues(options, {
        refresh: true,
        until: createFailure
          ? null
          : (issues) => matchingSliceIssues(issues, map, slice).length > 0,
      }),
      map,
      slice,
    );
